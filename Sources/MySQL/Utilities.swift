...
extension MySQL {
    ...
    internal struct Utils {
        ...
        static func skipLenEncStr(_ data: Data) -> Int {
        ...
        static func lenEncBin(_ b: Data) ->([UInt8]?, Int) {
        ...
        static func lenEncStr(_ b: Data) ->(String?, Int) {
            ...
            if b.count >= n {
                let s = b.startIndex
                let str = Data(b[s+n-Int(num!)...s+n-1])
                
                return (String(data: str, encoding: .utf8), n)
            }
        ...
        static func lenEncInt(_ b: Data) -> (UInt64?, Int) {
            ...
            case 0xfc:
                return (UInt64(b[s+1]) | UInt64(b[s+2])<<8, 3)
                
                // 253: value of following 3
            case 0xfd:
                return (UInt64(b[s+1]) | UInt64(b[s+2])<<8 | UInt64(b[s+3])<<16, 4)
 ...
 */
                var a = UInt64(b[s+1]) | UInt64(b[s+2])<<8 | UInt64(b[s+3])<<16
                a = a | UInt64(b[s+4])<<24 | UInt64(b[s+5])<<32
                a = a | UInt64(b[s+6])<<40
                a = a | UInt64(b[s+7])<<48 | UInt64(b[s+8])<<56
        ...
        static func encPasswd(_ pwd:String, scramble:[UInt8]) -> [UInt8]{
            ...
            let s1 = SHA1().calculate(for: uintpwd)
...
extension Data {
    ...
    func value<T> () -> T {
        ...
        if T.self == String.self {
            ...
            var arr = self.map { (elem) -> UInt8 in
                return elem
            }
            
            guard arr.count > 0 && arr.last == 0 else {
                return "" as! T
            }
           // arr.removeLast()
            
            return String(cString: &arr) as! T
        }
        
        return self.withUnsafeBytes { (p) -> T in
            p.baseAddress!.load(as: T.self)
        }
    }
...
/*public extension Date
{
    ...
    static func dateFrom (dateTimeString str: String) -> Date? {
        let dateStringFormatter = DateFormatter()
        dateStringFormatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
        
        let date = dateStringFormatter.date(from: str)
        return date
    }
